...
source("schinos_mirna_analysis_config.r")
...
option_list <- list(
  ...
  make_option(c("-b", "--adapter_library"), type="character",
              help="The position of the adapter library fasta file. \n\t\tFile path. (default = '/home/analysis/known_adapter_library.fa')",
              metavar="Path"),
  ...
  make_option(c("-g", "--path_ref_genome_prefix"),
              help="The indexed reference genome directory plus the prefix. \n\t\tText. (default = '/home/analysis/Genomes/hsa_GRCh38/Index/hsa_GRCh38')",
              metavar="Path"),
...
## If present, overwrite the input values with the user configuration
if ("user_config" %in% names(argv$options)) {
  source(argv$options[["user_config"]])
}

## If present, overwrite the input values with the CMD values
...
if (preprocess) {
  ## UMI and Adapter Extraction
  cat(paste0("=> UMI EXTRACTION... ",input_file_name,exp_name,"\n"))
  system(paste0(path_umi_tools, "umi_tools extract --extract-method=regex --3prime --bc-pattern='.+(?P<discard_1>AACTGTAGGCACCATCAAT){s<=2}(?P<umi_1>.{12})(?P<discard_2>.+)' -L ",input_file_name,exp_name,"_umi-extract.log -I ",file_in," -S ",input_file_name,"_cleared",exp_name,".fastq.gz"))
  file_in = paste0(input_file_name,"_cleared",exp_name,".fastq.gz")
  input_file_name = paste0(input_file_name,"_cleared")
  ...
  ## If adapter not provided, use DNApi inferrence
  if (adapter == "") {
    ...
    ## Iterate FastQC adapter results for potential adapter sequences
    if (grepl("RAW_INPUT",adapter)) {
      ...
      if (length(potential_adapters) > 0) {
        ...
        # Determine the orientation/type of the possible adapter
    ...
    ## If adapter "RAW_INPUT" provided or inferred by DNApi, no adapter is present in the dataset
    if (grepl("RAW_INPUT",adapter)) {
      ...
      ## Cross reference with known Adapter Library
  ...
  ## Adapter fragments detection and cleansing Loops
  reads_cleansed = 0
  ...
  if (adapter_exists) {
    ...
    if (adapter_content_percent < 70) {
      ...
      if (new_adapter_content_percent <= adapter_content_percent) {
        ...
        # replace cutadapt default output files
    ...
    ## Loop on reads without adapter using the adapter K-mer
    ...
    ## Merge cleansed reads to the pre-processed output
...
# Quality Check after Pre-process
...
cat(paste0("=> REFERENCE GENOME ALIGNMENT... ",input_file_name,exp_name,"\n"))
...
if (length(grep("Mapping statistics",readLines("mapping_log.txt"))) < 1) {
  stop("ALIGNMNET ERROR: Please refer to mapping_log.txt in the results directory for more information.")
}
mapping_table = tryCatch({
  ...
  stop("ALIGNMENT ERROR: Please refer to mapping_log.txt in the results directory for more information.")
})
# Check mapped percentage
if (mapping_table[2,5] < 0.5) {
  cat(">> ALIGNMENT WARNING: Mapped percentage to genome is less than 50% , consider checking your dataset or the reference genome provided.\n")
}

## Quantification using known miRNAs
...
if (preprocess) {
  ## Plot Read Length Distributions Before and After the analysis
